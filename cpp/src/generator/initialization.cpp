...
[[nodiscard]] AxisFrame build_axis_frame(
    ...
    return AxisFrame{.direction = direction, .u = u, .v = v, .axis_min = axis_min, .axis_max = axis_max};
}
...
[[nodiscard]] CandidateScore score_candidate(
    ...
    return CandidateScore{.length = polyline_length(points), .coverage = estimate_coverage(points, axis, frame)};
}
...
[[nodiscard]] std::vector<geometrycentral::surface::SurfacePoint> collect_seed_candidates(
    ...
    if (use_single_seed) {
        return {initialize_surface_seed_point(mesh, geometry, axis).inSomeFace()};
    }
...
InitializationResult select_best_generator_initialization(
    geometrycentral::surface::ManifoldSurfaceMesh& mesh,
    geometrycentral::surface::VertexPositionGeometry& geometry,
    const GeneratorAxis& axis,
    ...
    const std::vector<geometrycentral::surface::SurfacePoint> seed_points =
        collect_seed_candidates(mesh, geometry, axis, frame, options.use_single_seed);
    ...
    for (const geometrycentral::surface::SurfacePoint& seed_surface : seed_points) {
        ...
        try {
            step = bootstrap_from_seed(
                ...
                seed_surface,
                axis,
                options.initial_heading_angle_rad,
                spacing_world,
                max_curvature,
                curvature_tolerance
            );
    ...
    return InitializationResult{
        ...
        .evaluation_iterations = best_candidate->evaluation_iterations,
    };
}
