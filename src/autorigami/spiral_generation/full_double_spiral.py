...
class SpiralMiddleFunnel(SpiralObject):
    ...
    def get_point(self, position: float) -> Vector3:
        ...
        # We progressively distort the y radius by the square sine so it curves in
        y_radius = self.y_radius * (1.0 - (position * 0.95) * math.sin((angle - self.starting_angle) * 2))
        center = np.array([0.0, 0.0, self.starting_coords[2]], dtype=np.float32)
        x = x_radius * math.cos(angle) # x is projected outward
        y = y_radius * math.sin(angle) # y remains constant
        z = self.length * position
